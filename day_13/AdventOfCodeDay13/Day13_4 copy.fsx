...
let busses = parseInput2 textInput2
...
let checkNumber (busses: (double * double) list) (number: double) =
    busses
    |> List.map (fun (offset, schedule) -> (schedule - (offset % schedule), schedule))
    |> List.map (fun (rem, schedule) -> ((if rem = schedule then 0.0 else rem), schedule))

    |> List.forall (fun (rem, schedule) -> number % schedule = rem)
...
let scheduleBusses = parseInput2 scheduleText

[ 894954360381363.0 .. 894954360381404.0 ]
|> List.find (checkNumber scheduleBusses)
|> printfn "%f"

894954360381404.0 - 894954360381385.0 
894954360381385.0 - 894954360381363.0
