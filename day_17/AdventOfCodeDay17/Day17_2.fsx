type Cell = { X: int; Y: int; Z: int; W: int }
...
let neighbourIndexes =
    ...
    |> Array.except [ { X = 0; Y = 0; Z = 0; W = 0 } ]

neighbourIndexes |> Array.length

let getNeighbours { X = xC; Y = yC; Z = zC; W = wC } =
    neighbourIndexes
    |> Array.map (fun { X = x; Y = y; Z = z; W = w } ->
        { X = x + xC
          Y = y + yC
          Z = z + zC
          W = w + wC })
...
generation (getNeighbours { X = 0; Y = 0; Z = 0; W = 0 })

getNeighbours { X = 0; Y = 0; Z = 0; W = 0 }
|> Array.collect getNeighbours
|> Array.countBy id
// |> Array.length
// |> Array.head
...
|> List.collect (fun (x, chars) ->
    ...
    |> List.map (fun (y, _) -> { X = x; Y = y; Z = 0; W = 0 }))
|> List.toArray
|> generation
|> generation
|> generation
