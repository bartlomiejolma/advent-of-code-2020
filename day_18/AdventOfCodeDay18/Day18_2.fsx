...
let rec parseContent (content: string) =
    ...
    | _ ->
        ...
        | false -> 
            let multiplyMatch = Regex.Match(content, "(\d+ [*] \d+)")
            content.Contains("+") 
            |> function
            | false ->
                parseContent
                    (content.[0..multiplyMatch.Index - 1]
                     + (parseMatch multiplyMatch.Value)
                     + content.[multiplyMatch.Index + multiplyMatch.Length..])
            | _ -> content

Regex.Split("13 * 15 * 4 * 3", "(\d+ [*] \d+)")
Regex.Split("13 * 15 + ", "(\d+ [*] \d+)")
